...
```
```{r}
housingdata=read.csv("initial_dummied_data_TRAIN.csv")
housingdatatest=read.csv("initial_dummied_data_TEST.csv")
```
...
```{r}
sort(vif(forwardBIC),decreasing = TRUE)[sort(vif(forwardBIC),decreasing = TRUE)>7]
sort(vif(backwardBIC),decreasing = TRUE)[sort(vif(backwardBIC),decreasing = TRUE)>7]
sort(vif(bothBIC),decreasing = TRUE)[sort(vif(bothBIC),decreasing = TRUE)>7]
...
```{r}
model_both = lm(SalePrice ~ LotConfig_CulDSac+ExterQual_Gd+Neighborhood_IDOTRR+Condition1_Artery+BsmtQual_Gd +HeatingQC_TA+MSSubClass_multi_fam+BsmtFinGdLvng+BedroomAbvGr +Neighborhood_NoRidge+RemodelYrsAftBuilt   +MasVnrType_BrkFace+BldgType_TwnhsE +MasVnrArea+Functional_Min+BldgType_Twnhs+Functional_Maj+BsmtQual_Ex          +BsmtExposure +Exterior1st_BrkFace+OverallCondBinary+YearBuilt+TotalBsmtSF+BsmtUnfSF+ExterQual_Ex+OverallQual      +GrLivArea+LndAc_S, data = housingdata)
