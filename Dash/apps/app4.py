...
import dash_table
tdata=pd.read_csv('apps/tdata_for_realtor_map.csv')
...
def display_value(value):
    if value=='tab-1':
        return html.Div([
            ...
            ]),dcc.Markdown('''Introducing our Real Estate Handbook, a data rich review of Ames to help real estate agents better understand the market and further assist customers.
            Here you will find analysis of different house features' effect on price as found in our model, a look into the similar and different neighborhoods in Ames, as well as an ML generated sales comps tools for your convenience.
'''),html.Div([html.Img(src='shutterstock_752498263-1400x621.jpg')], style={'textAlign': 'center'}),
    elif value=='tab-2':
        return html.Div([html.H3('Lets get Ames-icable', style={'font-weight': 'bold'})]),dcc.Markdown('''Welcome to Ames, Iowa; named one of the “15 Cities That Have Done the Best Since the Recession” by Bloomberg Business in 2015 and known as the ‘healthiest city in America’ by USA Today. Home to Iowa State University, Ames has become a player in developing the world’s bio-economic future. An attractive location for business, some of the names housed here include 3M, Danfoss, Boehringer Ingelheim, and more. Below, see a simplified map of the houses sold in Ames between ____. Interacting with the map will give a general understanding of the city.'''), html.Div([html.H3('Visualize the City', style={'font-weight': 'bold'})]),dcc.Markdown('''Use this graph to visualize some of the trends of the neighborhood. Use the dropdown menu to color the points by different attributes.'''),html.Div([
    dcc.Dropdown(
        id='dropdown_for_realt_map',
        options=[{'label': 'Year Built','value': 'YearBuilt'},
        {'label': 'Gross Living Area', 'value': 'GrLivArea'} ,
                 {'label': 'Sale Price','value': 'SalePrice'},

                 {'label': 'Neighborhood','value': 'Neighborhood'},
                 {'label': 'Overall Quality','value': 'OverallQual'}],value='colorvalue')]),dcc.Graph(id='realt_map')
    elif value=='tab-5':
        return html.Div([html.H3('KNN Sales Comps', style={'font-weight': 'bold'})]),dcc.Markdown('''This tool uses K-Nearest Neighbors to generate automatic sales comps. Input attributes below to find houses most similar to it and how much they sold for. See below for accepted ranges.'''),dcc.Input(id='grlivarea',placeholder='Enter Gross Living Area',type='text',value=''),dcc.Input(id='lotarea',placeholder='Enter Lot Area value',type='text',value=''),dcc.Input(id='qualval',placeholder='Enter Quality value 1-10',type='text',value=''),dcc.Input(id='yearbuiltval',placeholder='Enter Year Built',type='text',value=''),dcc.Input(id='bsmtsfval',placeholder='Enter Basement SF',type='text',value=''),dcc.Input(id='garageareaval',placeholder='Enter Garage Area',type='text',value=''),dcc.Input(id='latval',placeholder='Enter Latitude',type='text',value=''),dcc.Input(id='longval',placeholder='Enter Longitude',type='text',value=''),dbc.FormGroup([dbc.Label("Remodeled?"),dbc.RadioItems(options=[{"label": "True", "value": 1},{"label": "False", "value":2},],value=1,id="remodeledtf",inline=True)]),dash_table.DataTable(id='knn_table')
@app.callback(
...
def return_knn(valuegr,valuela,valuequality,valueyearbuilt,valuebsmtsf,valuegarage,valueremodeled,valuelat,valuelong):
    tdata=pd.read_csv('apps/Data_For_KNN_salescomps.csv')
    Miles=2
    numofneighbors=10
