...
We performed SVR using the RBF and linear kernels. The RBF gave horrible results and performed worse than the null model according to the R2. The linear kernel performed well but again we opted for more interpretable models. The poly SVR was attemped although proved to be too computationally expensive. Note, the SVR was performed with the Lnd_Acs column which we eventually decided to drop. This could explain the R2 advantage.
'''
